...
| `polar_function`          | Sequence or `None`           | `None`               | Sequence of polar functions, one per aerodynamic surface. These map f(alpha, database) -> cl, cd, cm at the quarter chord. Individual entries can be set to `None` to have no polar correction for a single surface, or the full input can be `None` for no polar corections on the model. |
| `polar_data`              | Sequence or `None`           | `None`               | Database of polar data per aerodynamic surface with arbitrary type. `None` disables polar correction for all surfaces; set entry int he sequence to `None`to disable polar corrections for a single surface.                                                                               |
